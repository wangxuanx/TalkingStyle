...
@torch.no_grad()
def test(args, model, wav_path, epoch):
    dev = args.device
    ...
    for iter in range(one_hot_all.shape[-1]):
        ...
        audio = torch.from_numpy(audio).unsqueeze(0).to(dev)
        template = torch.from_numpy(template).unsqueeze(0).to(dev)
        one_hot = torch.from_numpy(one_hot).unsqueeze(0).to(dev)
        prediction = model.predict(audio, template, one_hot)
        prediction = prediction.squeeze() # (seq_len, V*3)
        np.save(os.path.join(result_path, "F2_e37_condition_"+condition_subject+".npy"), prediction.detach().cpu().numpy())
...
def main():
    ...
    parser.add_argument("--test_wav", type=str, default='/workspace/TalkingStyle/BIWI/wav/F2_e37.wav')
    ...
    demo_audio = args.test_wav
    
    test(args, model, demo_audio, epoch=90)
