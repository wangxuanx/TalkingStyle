...
def main():
    ...
    L2_dis_mouth_sum = np.array([np.square(vertices_gt_all[:, v, :]-vertices_pred_all[:, v,:]) for v in mouth_map])
    L2_dis_mouth_sum = np.transpose(L2_dis_mouth_sum, (1,0,2))
    L2_dis_mouth_sum = np.sum(L2_dis_mouth_sum,axis=2)
    ...
    print('Lip Motion Error: {:.4e}'.format(np.mean(L2_dis_mouth_mean)))
