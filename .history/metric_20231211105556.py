...
Traceback (most recent call last):
  File "/workspace/TalkingStyle/test_all_metric.py", line 90, in <module>
    main()
  File "/workspace/TalkingStyle/test_all_metric.py", line 50, in main
    motion_pred = vertices_pred - templates[subject].reshape(1,23370,3)
KeyboardInterruptimport numpy as np
import argparse
import os
import pickle
...
def main():
    ...
    parser.add_argument("--pred_path", type=str, default="/workspace/TalkingStyle/BIWI/save/result")
    parser.add_argument("--gt_path", type=str, default="/workspace/CodeTalker_first_test/BIWI/vertices_npy")
    ...
    result_list = os.listdir(args.pred_path)

    for model_id in result_list:
        list_pred = os.path.join(args.pred_path, model_id)
        print(f'Testing the {model_id}')
        ...
        for subject in train_subject_list:
            for sentence in sentence_list:
                ...
                vertices_pred = np.load(os.path.join(list_pred, subject+"_"+sentence+"_condition_"+subject+".npy")).reshape(-1,23370,3)
        ...
        L2_dis_mouth_sum = np.sum(L2_dis_mouth_max,axis=2)
        L2_dis_mouth_max = np.max(L2_dis_mouth_sum,axis=1)
        L2_dis_mouth_mean = np.mean(L2_dis_mouth_sum,axis=1)

        print('Lip Vertex Error: {:.4e}'.format(np.mean(L2_dis_mouth_max)))
        print('Lip Vertex Error: {:.4e}'.format(np.mean(L2_dis_mouth_mean)))
